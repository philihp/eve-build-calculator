...
const LOCAL_SDE_PATH = join(process.cwd(), "public", "sde", "mapStars.jsonl");
...
const BUCKET_KEY_PREFIX = "stars/bucket-";
...
async function buildFullMap(): Promise<Map<string, string>> {
  ...
  const res = await fetch(`${baseUrl()}/sde/mapStars.jsonl`);
  if (!res.ok) {
    throw new Error(
      `failed to fetch mapStars.jsonl from ${baseUrl()}: ${res.status}`,
    );
  }
...
export async function GET(
  ...
  const json = await fetchJson(starID);
