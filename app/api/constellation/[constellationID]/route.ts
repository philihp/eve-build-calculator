...
const BUCKET_KEY_PREFIX = "constellations/bucket-";
...
async function buildFullMap(): Promise<Map<string, string>> {
  ...
  const res = await fetch(`${baseUrl()}/sde/mapConstellations.jsonl`);
  if (!res.ok) {
    throw new Error(
      `failed to fetch mapConstellations.jsonl from ${baseUrl()}: ${res.status}`,
    );
  }
...
export async function GET(
  _req: Request,
  { params }: { params: Promise<{ constellationID: string }> },
) {
  const { constellationID } = await params;
  const json = await fetchJson(constellationID);
