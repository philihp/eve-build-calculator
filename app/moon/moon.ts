...
const asin = (x: number) => Math.asin(Math.min(1, Math.max(-1, x))) * DEG;
const acos = (x: number) => Math.acos(Math.min(1, Math.max(-1, x))) * DEG;
const atan2 = (y: number, x: number) => Math.atan2(y, x) * DEG;
const rev = (x: number) => ((x % 360) + 360) % 360;

export type MoonPosition = {
  elevation: number; // topocentric altitude above the horizon (deg)
  azimuth: number; // compass bearing, clockwise from north (deg)
  distanceKm: number; // centre-to-centre distance (km)
  illumination: number; // illuminated fraction of the disc (0..100 %)
  phase: string; // named phase
};

const phaseName = (age: number): string => {
