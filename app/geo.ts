...
const reverseGeocode = async (
  ...
  try {
    const res = await fetch(
      ...
      {
        cache: "no-store",
        headers: { "User-Agent": "edencom-sde/sun-moon-pages" },
      },
    );
...
export type ResolvedLocation = {
  // Vercel's IP-based estimate.
  lat: number | null;
  lon: number | null;
  place: string; // "" when unknown
  // Browser-shared device location, if the visitor opted in.
  deviceLat: number | null;
  deviceLon: number | null;
  devicePlace: string | null;
  // Device location when shared, IP estimate otherwise.
  effLat: number | null;
  effLon: number | null;
  usingDeviceLocation: boolean;
};

export const resolveLocation = async (): Promise<ResolvedLocation> => {
